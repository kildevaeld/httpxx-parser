...
template <typename T> class Emitter {
  ...
  template <typename E> struct Handler final : BaseHandler {
  ...
  template <typename> static std::size_t event_type() noexcept {
    static std::size_t value = next_type();
    return value;
  }

  template <typename E> Handler<E> &handler() noexcept {
...
public:
   ...
   * type is registered.
   */
  template <typename E> struct Connection : private Handler<E>::Connection {
   ...
   */
  template <typename E> Connection<E> on(Listener<E> f) {
   ...
   * @return Connection object to be used later to disconnect the listener.
   */
  template <typename E> Connection<E> once(Listener<E> f) {
   ...
   */
  template <typename E> void erase(Connection<E> conn) noexcept {
   ...
   */
  template <typename E> void clear() noexcept { handler<E>().clear(); }
   ...
   * false otherwise.
   */
  template <typename E> bool empty() const noexcept {
    std::size_t type = event_type<E>();
